...
import os, sys, redis, json, re, struct, time, socket, argparse, psutil, string
...
def _get_vmsize():
    ...
    regex = "pid=(\d+),\sname=\'" + "redis-server" + "\'"
    pid = 0
    for line in p:
        ...
        if result != None:
            ...
            if pid:
                fn = os.path.join('/proc', str(pid), 'status')
                with open(fn) as fd:
                    for line in fd:
    ...
    if redis_hostname and args.metric:
        ...
        if args.metric:
            ...
            def _vmsize_ratio():
                ...
                print round(float(vmmem*1024) / total_mem, 2)
